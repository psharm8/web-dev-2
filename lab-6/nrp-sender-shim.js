...
const sendMessage = (messageConfig = defaultMessageConfig) => {
    return new Promise((fulfill, reject) => {
        ...
        let successEventName = undefined;
        let failedEventName = undefined;
        if (settings.expectsResponse) {
            successEventName = `${eventName}:success:${messageId}`;
            failedEventName = `${eventName}:failed:${messageId}`;
            ...
            if (settings.timeout >= 0) {
                killswitchTimeoutId = setTimeout(() => {
                    reject({ code: 408, message: "Timed out!" });
                    endMessageLifeCycle();
                }, settings.timeout);
            }
        }
